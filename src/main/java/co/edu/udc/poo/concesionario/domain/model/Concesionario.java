...
public class Concesionario {
    ...
    public List<Vehiculo> getVehiculos(){
        ...
        VehiculosDisponibles.addAll(bodega.getVehiculosDisponiblesInSitu());
    ...
    public void agregarVehiculo(String nif,String ubicacion, Vehiculo vehiculo){
        ...
        switch (ubicacion) {
            ...
            case "servicioOficial":
                ...
                break;
            case "bodega":
                bodega.getVehiculosDisponiblesInSitu().add(vehiculo);
                break;
            default:
    ...
    public double calcularVentaTotalVendedor(String nif){
        ...
        for(Venta venta: ventas){
            if(venta.getVendedor().nif.equals(nif)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
            }
        }
        return total;
    }

    public List<Venta> ventasPorVendedor(String nif){
    ...
    public double ventaMayorVendedor(String nif){
        ...
        for(Venta venta: ventas){
            if(venta.getVendedor().nif.equals(nif)){
                if(Double.parseDouble(venta.getPrecio().getPrecioFinal().toString()) > mayor){
                    mayor = Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
                }
            }
        }
        return mayor;
    ...
    public double ventasPorMarca(String marca){
        double total = 0;
        for(Venta venta: ventas){
            if(venta.getVehiculo().Marca.equals(marca)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
            }
        }
        return total;
    ...
    public double ventasPorCarroceria(String carroceria){
        double total = 0;
        for(Venta venta: ventas){
            if(venta.getVehiculo().carroceria.equals(carroceria)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
            }
        }
        return total;
    ...
    public double ventasPorModelo(String modelo){
        double total = 0;
        for(Venta venta: ventas){
            if(venta.getVehiculo().Modelo.equals(modelo)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
            }
        }
        return total;
    ...
    public String modeloMasVendido(){
        ...
        for(Vehiculo vehiculo: getVehiculos()){
            double total = 0;
            for(Venta venta: ventas){
                if(venta.getVehiculo().Modelo.equals(vehiculo.Modelo)){
                    total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
                }
            }
            if(total > mayor){
    ...
    public ArrayList<Venta> ventasPorCiudad(String ciudad){
        ArrayList<Venta> resultado = new ArrayList<>();
        for(Venta venta: ventas){
            if(venta.getVendedor().ciudad.equals(ciudad)){
                resultado.add(venta);
            }
        }
        return resultado;
    ...
    public double ventasPorDepartamentoTotal(String departamento){
        double total = 0;
        for(Venta venta: ventas){
            if(venta.getVendedor().departamento.equals(departamento)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
            }
        }
        return total;
    }
    public List<Venta> ventasPorDepartamento(String departamento){
        List<Venta> resultado = new ArrayList<>();
        for(Venta venta: ventas){
            if(venta.getVendedor().departamento.equals(departamento)){
                resultado.add(venta);
    ...
    public double ventasPorMetodoPago(String metodoPago){
        ...
        for(Venta venta: ventas){
            switch(metodoPago){
                case "contado":
                    System.out.println();
                    if(Double.parseDouble(venta.getPrecio().toString()) > 0){
                        total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
                    }
                    break;
                case "credito":
                    ...
                    break;
                case "mixto":
                    if(Double.parseDouble(venta.getPrecio().toString()) > 0 && Double.parseDouble(venta.getPrecio().toString()) > 0){
                        total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
                    }
                    break;
            }
    ...
    public double ventasPorAñoFabricacion(String anio){
        double total = 0;
        for(Venta venta: ventas){
            if(venta.getVehiculo().añoFabrica.equals(anio)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
            }
        }
        return total;
    ...
    public List<MetodoPago> metodosDePagoPorCliente(String cedula){
        ...
        for(Venta venta: ventas){
            if(venta.getCliente().getCedula().equals(cedula)){
                metodosDePago.add(venta.getMetodoPago());
            }
        }
    ...
    public List<Integer> satisfacciónPorCliente(){
        ...
        for(Cliente cliente: listaClientes){
            if(cliente.getNivelSatisfaccion().ordinal() >= 0){
                satisfaccion.add(cliente.getNivelSatisfaccion().ordinal());
            }
        }
        return satisfaccion;
    }
    ...
    public List<Cliente> clientesReferidos(){
        ...
        for(Cliente cliente: clientes){
            if(cliente.getReferidoPor() != null){
    ...
    public Cliente clienteConMasCompras(){
        ...
        int mayor = 0;
        for(Cliente cliente: listaClientes){
            if(cliente.getVentasRealizadas().size() > mayor){
                mayor = cliente.getVentasRealizadas().size();
    ...
    public double promedioDeCompraPorCliente(String nif){
        ...
        int contador = 0;
        for(Venta venta: ventas){
            if(venta.getCliente().getCedula().equals(nif)){
                total += Double.parseDouble(venta.getPrecio().getPrecioFinal().toString());
                contador++;
            }
        }
    ...
    public List<Vehiculo> vehiculosVendidosPorFecha(Date fechaI, Date fechaF){
        ...
        for(Venta venta: ventas){
            if(venta.getFechaEntrega().equals(fechaI) && venta.getFechaEntrega().equals(fechaF)){
    ...
    public List<String> coloresMasVendidos(){
        ...
        int c5 = 0;
        for(Venta venta: ventas){
            if(venta.getVehiculo().color.equals("negro")){
                c1++;
            }else if(venta.getVehiculo().color.equals("rojo")){
                c2++;
            }else if(venta.getVehiculo().color.equals("azul")){
                c3++;
            }else if(venta.getVehiculo().color.equals("verde")){
                c4++;
            }else if(venta.getVehiculo().color.equals("amarillo")){
                c5++;
            }
        }
    ...
    public List<Venta> ventasSuperiorA(double precio){
        ...
        for(Venta venta: ventas){
            if(Double.parseDouble(venta.getPrecio().getPrecioFinal().toString()) >= precio){
                ventas.add(venta);
            }
        }
        return ventas;
    ...
    public double porcentajeDeSatisfaccionClientes(){
        ...
        for(Cliente cliente: listaClientes){
            if(cliente.getNivelSatisfaccion().ordinal() > 0){
                satisfaccion += cliente.getNivelSatisfaccion().ordinal();
                contador++;
            }
        }
    ...
    public ArrayList<Integer> extrasMasVendidos() {
        ...
        int extra5 = 0;
        for (Venta venta : ventas) {
            if (venta.getExtras().size() == 1) {
                extra1++;
            } else if (venta.getExtras().size() == 2) {
                extra2++;
            } else if (venta.getExtras().size() == 3) {
                extra3++;
            } else if (venta.getExtras().size() == 4) {
                extra4++;
            } else if (venta.getExtras().size() == 5) {
                extra5++;
            }
        }
